...
梯度下降相关介绍：
    ...
    存在的问题：
        ...
        3.可能会遇到 局部最小值
    解决思路：
        从上述的 学习率 或者 梯度入手 进行优化，于是有了动量法Momentum ,自适应学习率AdaGrad,RMSProp,综合衡量：Adam
    ...
    自适应学习率：
        公式：
            ...
            学习率：
                学习率 = 学习率 / （sqrt（St）+ 小常数）
                解释：
                    小常数： 1e（-10次方）， 目的：防止分母变为0
                梯度下降公式：
                    W新 = W旧 - 调整后的学习率 * Gt
        缺点：
            可能会导致学习率过早，过量的降低，导致模型后期学习率太小，较难找到最优解

    自适应学习率RMSProp -> 可以看作是 对AdaGrad做的优化，加入 调和权重系数
        公式：
            累计平方梯度：
                St = β * St-1 +（1 - β）Gt * Gt
            解释：
    ...
    自适应矩阵估计：Adam（Adaptive Moment Estimation）
        思路：
            即优化学习率，又优化梯度
        公式：
            一阶矩：算均值
...
def dm01_momentum():
    ...
    #5.1 定义损失函数
    criterion = ((w ** 2) / 2.0)
    #5.2计算梯度值：梯度清零+反向传播+参数更新
    optimizer.zero_grad()
    criterion.sum().backward()
    optimizer.step()
    #5.3打印结果
    print(f'w:{w},w.grad:{w.grad}')
#2.定义函数演示：梯度下降法-> 自适应学习率（AdaGrad）
def dm02_AdaGrad():
    #1.初始化权重参数
    w = torch.tensor([1.0],requires_grad=True,dtype=torch.float)
    #2.定义损失函数
    criterion = ((w**2)/2.0)
    #3.创建优化器（函数对象）
    # 思路一：基于SGD（随机梯度下降），加入参数momentum，就是 动量法
    #参1：（待优化的）参数列表，参2：学习率，参3：动量参数。
    #optimizer = optim.SGD(params=[w],lr=0.01,momentum=0.9)#细节：momentum=0(默认)，只考虑：本次梯度
