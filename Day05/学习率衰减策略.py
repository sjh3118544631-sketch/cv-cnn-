...
学习率衰减策略介绍：
    目的：
        较之于AdaGrad,RMSProp,Adam方式，我们可以通于 等间隔 ，指定间隔，指数等方式，来手动控制学习率的调整
...
def dm01():
    ...
    #参1.待优化的参数，参2：学习率，参3：动量因子
    optimizer = optim.SGD([w],lr=lr,momentum=0.9 )

    #4.创建学习率衰减对象
    #参1：模型参数，参2：间隔的轮数，参3：学习率衰减系数
    scheduler = optim.lr_scheduler.StepLR(optimizer,step_size=50,gamma=0.5)

    #5.创建两个列表，分别表示：训练轮数，每轮训练用的学习率
    #epoch_list = [0,1,2----50-----100------199]
    #lr_list =    [0.1,0.1,--0.05------0.025,---0.0125]
    lr_list,epoch_list = [],[]

    #6.循环遍历训练轮数，进行具体的训练
    for epoch in range(epoch):    #epoch:0~199
        #7.获取当前轮数 和 学习率，并且保存到列表中
