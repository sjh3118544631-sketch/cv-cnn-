...
from torch.utils.data import Dataset          #数据加载器
...
def create_dataset():
    ...
    x,y,coef = make_regression(
        ...
        noise= 10,       #噪声，噪声越大，样本点越散反之越集中
        coef= True,      #是否返回系数，默认为false返回值为none
        bias=14.5,        #偏执
        random_state= 3  #随机种子 ，随机种子相同输出的结果相同
    )
    #1.2.把上述数据，封装成张量对象
...
def train(x,y,coef):
    ...
    for epoch in range(epochs): #epoch的值 ：0，1，2……99
        #6.3每轮是分 批次 训练的 所以从数据加载器中 获取 批次数据
    ...
    #9.3 绘制真实值
    y_ture = torch.tensor(data = [v * coef + 14.5 for v in x])
    #9.4 绘制预测值 和 真实值 折线图
