...
张量：
    ...
    张量-> 存储同一类型元素的容器，且元素值必须是整数才可以
张量的创建方法：
        torch.tensor  根据指定数据来创建张量
        torch.Tensor  根据形状创建张量，也可以用来创建指定数据的张量
        torch.IntTensor、torch.FloatTensor、torch.DoubleTensor 创建指定类型的张量
细节：
    Tensor相较于tensor方式，可以基于形状来直接创建张量（但是常用的还是tensor）
掌握：
tensor(值，类型)例如：
...
import torch
import numpy as np
#1.定义函数，演示：torch.tensor  根据指定数据来创建张量
def dm01():
    #场景一标量 张量
    t1 = torch.tensor(10)
    ...
    print(f"t3:{t3}，type:{type(t3)}")
    print("-" * 30)
    #场景四：尝试直接创建 指定维度（例如2行3列的）张量(会报错)
    #t4 = torch.tensor(2,3)
